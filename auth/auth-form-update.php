...
if (isset($_GET['code'])) {
    if (isset($_GET['selected_provider'])) {
        ...
        $stmt = $con->prepare('SELECT * FROM users WHERE acc_provider_id = ? AND acc_type = ?');
        $stmt->bind_param('ss', $provider_id, $selected_provider);
        $stmt->execute();
        ...
        if ($result) {
            ...
            addNotification($con, $user_id, "Connexion réalisée avec succès !", "Succès", $no_connect_user_id, $no_connect_ip);
        } else {
            $account_type = $selected_provider;
            $random_number = rand(1, 99999);

            $stmt = $con->prepare('INSERT INTO users (acc_username, acc_email, acc_type, acc_logo, acc_affiliate_id, acc_provider_id) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->bind_param('ssssss', $username, $primary_email, $account_type, $avatar_url, $random_number, $provider_id);
            $stmt->execute();
            $user_id = $stmt->insert_id;

            // Vérifier si un parrainage est en cours
            if (isset($_SESSION['parrainage_id'])) {
                $parrainage_id = $_SESSION['parrainage_id'];
                $parrainage_valeur = false;

                // Préparer la requête SQL pour vérifier l'existence du parrainage_id
                $sql = "SELECT id FROM users WHERE acc_affiliate_id = ?";
                ...
                if ($stmt->num_rows > 0) {
                    // Récupérer user_id et le stocker dans $parrainage_user_id
                    $stmt->bind_result($parrainage_user_id);
                    $stmt->fetch();
                    $parrainage_valeur = true;
                }

                // Insérer la valeur de parrainage_user_id dans la colonne parrainage_id de la table users
                if ($parrainage_valeur) {
                    $update_sql = "UPDATE users SET parrainage_id = ? WHERE id = ?";
                    $stmt_update = $con->prepare($update_sql);
                    $stmt_update->bind_param("ii", $parrainage_user_id, $user_id);
                    $stmt_update->execute();
                }
            }
            ...
            $user_id = $_SESSION['user_id'];
            addNotification($con, $user_id, "Inscription réalisée avec succès !", "Succès", $no_connect_user_id, $no_connect_ip);
        }
